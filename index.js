...
// ================== Dummy Data ==================
let users = []; // signup/login users
let patients = [{ id: 1, name: "Aman", age: 22 }];
let doctors = [{ id: 1, name: "Dr. Smith", specialization: "Cardiology" }];
let appointments = [];
let prescriptions = [];
...
// ================== Patients ==================
// Doctors/Admin can view all patients
app.get(
  "/patients",
  authenticateToken,
...
app.get(
  ...
  (req, res) => {
    res.json({ id: 99, name: req.user.username, age: 30 });
  }
);
...
app.get(
  ...
  (req, res) => {
    const myAppointments = appointments.filter(
      (a) => a.doctorId === req.user.username || a.doctor === req.user.username
    );
    res.json(myAppointments);
  }
);
...
app.get(
  ...
  authorizeRoles("patient"),
  (req, res) => {
    if (req.user.username !== req.params.patient) {
      return res
        .status(403)
        .json({ message: "Access denied: not your prescriptions" });
    }
    const myPrescriptions = prescriptions.filter(
      (p) => p.patient === req.user.username
    );
    res.json(myPrescriptions);
  }
);
...
// ================== Root ==================
app.get("/", (req, res) => {
  res.send("🚀 Telemedicine Backend Prototype Running");
});
